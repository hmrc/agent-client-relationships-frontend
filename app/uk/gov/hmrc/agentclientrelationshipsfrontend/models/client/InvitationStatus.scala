...
case object Accept extends InvitationStatus
...
object InvitationStatus extends Logging {
  ...
  def unapply(status: InvitationStatus): String = status match {
    case Pending      => "Pending"
    case Rejected     => "Rejected"
    case Accept     => "Accept"
    case Cancelled    => "Cancelled"
  ...
  implicit val format: Format[InvitationStatus] = new Format[InvitationStatus] {
