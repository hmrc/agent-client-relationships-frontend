    ...
    Form(
      single(
        clientDetail.inputType match {
          case "date" =>
            clientDetail.name -> dateFieldMapping(s"client-details.${clientDetail.name}")
          case _ =>
            clientDetail.name -> textFieldMapping(clientDetail.name, s"client-details.${clientDetail.name}", clientDetail.regex)
        }
      )
    )
