...
class RequestSupport @Inject() {
  ...
  implicit def headerCarrier(implicit request: Request[?]): HeaderCarrier = RequestSupport.headerCarrier
}

object RequestSupport {
  implicit def headerCarrier(implicit request: Request[?]): HeaderCarrier = HcProvider.headerCarrier
