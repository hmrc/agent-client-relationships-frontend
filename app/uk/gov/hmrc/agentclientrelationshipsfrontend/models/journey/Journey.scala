...
case class Journey(journeyType: JourneyType,
  ...
  def getExitType(journeyType: JourneyType, clientDetails: ClientDetailsResponse, supportsAgentRoles: Boolean = false): Option[JourneyExitType] = journeyType match
    case JourneyType.AuthorisationRequest => clientDetails match {
      ...
      case ClientDetailsResponse(_, None, _, _, _, false, Some(service)) if !supportsAgentRoles && service == clientService.get => Some(JourneyExitType.AuthorisationAlreadyExists)
