...
import uk.gov.hmrc.agentclientrelationshipsfrontend.config.{AppConfig, ExternalUrls, FeatureFlags}
import uk.gov.hmrc.agentclientrelationshipsfrontend.connectors.AgentClientAuthorisationConnector
import uk.gov.hmrc.agentclientrelationshipsfrontend.controllers._
import uk.gov.hmrc.agentclientrelationshipsfrontend.models._
import uk.gov.hmrc.agentclientrelationshipsfrontend.util.{AuthRedirects, CallOps}
...
class AuthActions @Inject()(
        ...
        case Some(affinity) ~ confidence ~ enrols ~ maybeNino =>
          (affinity, confidence) match {
            ...
            case (AffinityGroup.Agent, _) => {
              Future successful Redirect(clientInvitation.routes.InvitationErrorController.cannotViewRequest)
            }
  ...
  private def withConfidenceLevelUplift[A, BodyArgs](currentLevel: ConfidenceLevel, enrols: Enrolments)(body: => Future[Result])(
    ...
    if (currentLevel >= requiredCL || isCgtOnlyClient) {
      ...
      Future.successful(Redirect(clientInvitation.routes.InvitationErrorController.showCannotConfirmIdentity().url))
    }
  }
  ...
  private def redirectToIdentityVerification[A]()(implicit request: Request[A]) = {
    ...
    val rawFailureUrl =
      toLocalFriendlyUrl(clientInvitation.routes.InvitationErrorController.showCannotConfirmIdentity().url, request.host)
  ...
  private def continueUrlWithJourneyId(journeyId: Option[String])(implicit request: Request[_]): String = {
    ...
    journeyId.fold(url)(_ => CallOps.addParamsToUrl(url, "clientInvitationJourney" -> journeyId))
  }
