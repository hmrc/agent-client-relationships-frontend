...
//  private lazy val nextPageUrl: String = routes.CheckYourAnswersController.show.url
  ...
  def show: Action[AnyContent] = actions.getJourney.async:
    journeyRequest =>
      ...
      Future.successful(Ok(view(form, agentTypes, previousPageUrl, clientName)))
  ...
  def onSubmit: Action[AnyContent] = actions.getJourney.async:
    journeyRequest =>
      ...
      SelectFromOptionsForm.form(AgentTypeFieldName, agentTypes).bindFromRequest().fold(
        ...
        agentType => {
          journeyService.saveJourney(journeyRequest.journey.copy(agentType = Some(agentType))).flatMap { _ =>
