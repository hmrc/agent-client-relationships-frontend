  ...
  def nextPageUrl(journeyType: AgentJourneyType)(implicit request: Request[Any]): Future[String] = {
    for {
      ...
      case Some(journey) =>
        ...
        else if ((journey.clientDetailsResponse.get.knownFactType.nonEmpty && journey.knownFact.isEmpty) || !journey.isKnowFactMatching)  routes.EnterClientFactController.show(journeyType).url
