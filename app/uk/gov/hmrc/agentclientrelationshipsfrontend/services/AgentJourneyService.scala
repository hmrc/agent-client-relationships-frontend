    ...
    for {
      ...
      case Some(journey) =>
        ...
        if (journey.clientService.isEmpty) {
          ...
          routes.CbcMissingKnownFactsController.show(journeyType).url // This is a temporary page to handle the case where we know the client has missing known facts, and so cannot proceed with the journey.
