...
import uk.gov.hmrc.agentclientrelationshipsfrontend.models.client.{AgentData, ManageYourTaxAgentsData, Pending}
...
class ManageYourTaxAgentsPageSpec extends ViewSpecSupport {
  ...
  val mytaData = ManageYourTaxAgentsData(
    currentRequests = Seq(
      AgentData(
        uid = "NBM9TUDA",
        agentName = agentName,
      ...
      doc.mainContent.extractTable(1, 4).value shouldBe TestTable(
        ...
        rows = List(
          IndexedSeq(
            ...
            s"Respond to request from $agentName to manage your VAT as a Main agent"
          ),
          IndexedSeq(
  ...
  "ManageYourTaxAgentsPage view without any current requests" should {
    ...
    val view: HtmlFormat.Appendable = viewTemplate(services, mytaData.copy(currentRequests = Seq.empty))
    val doc: Document = Jsoup.parse(view.body)
