...
class EnterClientFactControllerISpec extends ComponentSpecHelper with AuthStubs {
  ...
  "POST /authorisation-request/client-fact" should {
    ...
    "redirect to known-fact-not-matched when submitting a mismatching KF" in {
      ...
      result.status shouldBe SEE_OTHER
      result.header("Location").value shouldBe routes.JourneyErrorController.show(JourneyType.AuthorisationRequest, "known-fact-not-matched").url
    }
  ...
  "POST /agent-cancel-authorisation/client-fact" should {
    ...
    "redirect to known-fact-not-matched when submitting a mismatching KF" in {
      ...
      result.status shouldBe SEE_OTHER
      result.header("Location").value shouldBe routes.JourneyErrorController.show(JourneyType.AgentCancelAuthorisation, "known-fact-not-matched").url
    }
    "show an error when no selection is made" in {
      authoriseAsAgent()
      await(journeyService.saveJourney(testItsaJourney(JourneyType.AgentCancelAuthorisation)))
